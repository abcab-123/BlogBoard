...
@EnableWebSecurity // 스프링 시큐리티를 활성하고 웹 보안 설정을 구성하느데 사용 , websecurityConfiguereApdapter 클래스를 상속한 구성
					// 클래스에서 사용됨
	...
	SecurityFilterChain fliterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(a -> {
			a.requestMatchers(RegexRequestMatcher.regexMatcher("/board/\\d+" + "/dummy/\\d+")).permitAll()
					.requestMatchers("/users/**", "/board/**").authenticated().anyRequest().permitAll();
		});

		http.formLogin(f -> {
			f.loginPage("/auth/loginForm").loginProcessingUrl("/auth/loginProc").defaultSuccessUrl("/")
